...
class UserModel extends CI_Model {
	...
	{  
 		...
 		if($this->session->userdata['Admin']['role']!="Stockist")
			$this->db->from($this->User);
		else
			$this->db->from("stockist");
	...
	{  
 		$this->db->select('id,name,mobile_no,email,address,city,country,vat_number,picture_url,pincode,created,pancard');
		$this->db->from($this->User);
		$this->db->where("id",$id);
		$this->db->limit(1);
  		$query = $this->db->get();
