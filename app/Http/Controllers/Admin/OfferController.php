...
class OfferController extends Controller
{
    ...
    public function load_data(Request $request)
    {
      ...
      $data_list = Offer::where(['status' => $status])->orderBy('id',$order_by);
            if(!empty($filter_search_value))
      {
        $filter_search_value = explode(" ", $filter_search_value);
        foreach ($filter_search_value as $key => $value)
        {
            $data_list = $data_list->where('title','LIKE',"%{$value}%");            
        }
      }
    ...
    public function update(Request $request)
    {
        ...
        if(empty($id)) $data = new Offer;
        else $data = Offer::find($id);
