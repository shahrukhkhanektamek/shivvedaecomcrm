...
class CouponController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $data['submit_date_url'] = route($this->arr_values['routename'].'submit_date');
    ...
    public function load_data(Request $request)
    {
      ...
      $data_list = Coupon::where(['status' => $status])->orderBy('id',$order_by);
      ...
      if(!empty($filter_search_value))
      {
        ...
        {
            $data_list = $data_list->where('code','LIKE',"%{$value}%");            
        }
      }
    ...
    public function update(Request $request)
    {
        ...
        if(empty($id)) $data = new Coupon;
        else $data = Coupon::find($id);
        ...
        $data->limit_use = $request->limit_use;
        ...
        if($data->save())
        {
            ...
            $old_slug = $data->slug;
            $insert_id = $data->id;        
            $data->slug = Helpers::insert_slug($new_slug,$insert_id,"blog",$old_slug);
    ...
    public function submit_date(Request $request)
    {
        $session = Session::get('admin');
        $add_by = $session['id'];        
        ...
        if(DB::table('coupon')->update($data))
        {
            $action = 'add';
            if(!empty($id)) $action = 'edit';
            $responseCode = 200;
